...
async def register_drug_by_image(file: UploadFile = File(...), mode: str = Form("prescription")):
    ...
    try:
        ...
        for pill_name in detected_pills:
            if pill_name and pill_name != "Unknown":
                # [주의] 실제 앱에선 user_id를 프론트에서 받아야 함. 지금은 테스트용 고정.
...
def consult_drug_by_name(request: ConsultByNameRequest):
    ...
    try:
        mock_req = ConsultationRequest(
            class_id=0,
            user_profile=UserProfile(symptom="약품 문의", age=30, condition="일반"),
...
async def analyze_food(file: UploadFile = File(...)):
    ...
    try:
        # [수정] DB에서 약 목록 가져오기
        drug_list = get_user_drug_list(user_id="test_user") 
